...
{
	struct dma_heap_attachment *a = attachment->priv;
	struct coherent_heap_buffer *buffer = attachment->dmabuf->priv;
...
static int coherent_heap_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	return dma_mmap_coherent(dma_heap_get_dev(coh_heap->heap), vma,
				 buffer->alloc_vaddr, buffer->dma_addr,
				 buffer->len);
}
...
static int __coherent_heap_register(struct reserved_mem *rmem)
{
	...
	coh_heap->no_map = !!of_get_flat_dt_prop(rmem->fdt_node, "no-map", NULL);
