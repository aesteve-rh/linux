			  ...
			  struct cma *cma)
{
	...
	/* CMA pages each have a 0-order folio, mark it with the memcg */
	objcg = get_obj_cgroup_from_memcg(memcg);
			     ...
			     struct cma *cma)
{
	...
	/*
	 * Drop the objcg ref taken during charge_cma.
	 * Clean memcg_data, they're all the same objcg.
	 */
	for (i = 0; i < count; ++i) {
